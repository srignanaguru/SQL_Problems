...
-- To find out the average amount users have spend per order on all friday

CREATE TABLE user_purchases(user_id int, date date, amount_spent float, day_name varchar(15));

INSERT INTO user_purchases
VALUES(1047,'2023-01-01',288,'Sunday'),
...
select avg(amount_spent) as user_per_order_avg,year(date) as year,month(date) as month, day(date) as date,
	date_format(date,"%W") as wotd, week(date) as week
from user_purchases group by year(date),month(date),day(date),wotd,week(date) having wotd='Friday';
